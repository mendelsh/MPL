...
/* Test 13: Extreme growth with gigabyte allocations */
void test_gigabyte_allocations() {
    printf("Test 13: Gigabyte-scale allocations...\n");
    arena_fd_t *arena = afd_init(32, ram_threshold);
    
    // Allocate increasingly large chunks up to 1GB
    for (int i = 1; i <= 5; i++) {
        size_t size = (size_t)i * 256 * 1024 * 1024; // 256MB, 512MB, 768MB, 1GB, 1.25GB
        void *ptr = afd_get_memory(arena, size);
        if (ptr) {
            // Touch every 64MB to ensure mapping
            for (size_t offset = 0; offset < size; offset += 64 * 1024 * 1024) {
                ((char*)ptr)[offset] = (char)i;
            }
            printf("    Allocated and verified %zu MB\n", size / (1024 * 1024));
        } else {
            printf("    Failed to allocate %zu MB (might be system limit)\n", size / (1024 * 1024));
        }
    }
